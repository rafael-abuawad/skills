...
Follow control across `extcall`, `staticcall`, raw-call success/failure branches,
`@raw_return`, `__default__`, module calls/exports, factories, and callbacks. A
Vyper typed call can be internally well formed while an empty/defaulted return,
fee-on-transfer delta, stale oracle, raw response truncation, or imported-module
state violates protocol intent. Include compiler-version context whenever the seam
relies on legacy code generation rather than source-level behavior.
