...
Map trust through inline assertions, snekmate modules, `initializes:`/`uses:`
dependency ownership, `exports:` exposure, factory/blueprint deploy authority, and
raw-delegatecall implementation control. Check whether different Vyper files have
different nonreentrancy pragmas or whether `@reentrant` creates a privileged
callback route. Express the economic leg with actual `extcall`/`raw_call` token or
oracle semantics, not an assumed Solidity wrapper.
