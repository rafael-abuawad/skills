...
For each seam, include Vyper `decimal` (10 places), checked `convert`, checked
normal arithmetic, and `unsafe_*`/`pow_mod256` explicitly. A checked overflow is
a possible liveness boundary, not silent value creation. A conversion is not a
Solidity-style wrap unless a subsequent operation or representation change proves
one. Use `//` truncation and actual token/oracle scales in numeric proofs.
